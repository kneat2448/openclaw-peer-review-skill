...
.env.example
package.json
skills/peer-review-telegram/subagent.manifest.json — machine-readable subagent contract
skills/peer-review-telegram/references/subagent.md — OpenClaw subagent runbook
```
...
2. For subagent handoff, read `subagent.manifest.json` and `references/subagent.md`.
3. Inspect `.env`, `data/company_profile.json`, `data/team_members.json`, and `src/bot.js` before changing anything.
4. After any code change, run:
...
5. Restart: `npm start` or `pm2 restart peer-review-bot`.
6. Verify `GET /` responds and Telegram polling logs show started.
